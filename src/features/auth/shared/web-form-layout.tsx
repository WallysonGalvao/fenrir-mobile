...
type WebFormLayoutProps = PropsWithChildren<{
  title: string;
  subtitle?: string;
}>;

export function WebFormLayout({ title, subtitle, children }: WebFormLayoutProps) {
  return (
    <Animated.View
    ...
    >
      ...
      <View className="flex-1 flex-row">
        ...
        <View className="flex-6 bg-background-surface-alt dark:bg-background-surface-alt-dark justify-center items-start px-16">
          <View className="max-w-120 w-full gap-6">{children}</View>
        </View>
      </View>
    </Animated.View>
  );
